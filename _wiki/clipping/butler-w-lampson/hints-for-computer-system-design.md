...
이런 사례들은 이더넷 local 네트워크와 Alto, Dorado 개인용 컴퓨터 같은 하드웨어부터 SDS 940와 Alto 운영체제, Lisp, Mesa 같은 프로그래밍 시스템, Bravo 에디터, Star 오피스 시스템, Dover 프린터, Grapevine 메일 시스템 같은 응용 프로그램, 네트워크 서버까지 다양할 것입니다.
...
네가 갖고 있는 친구들, 그들의 성향을 시험해 본 후,  
강철의 고리로 너의 영혼에 꽉 붙잡거라;  
하지만 새로 사귄 미숙한 친구들과의 교제로  
손을 무디게 하지는 말거라.
...
- 다른 디렉토리에 대한 엑세스 권한을 얻기 위한 시스템 호출 `CONNECT`가 있습니다. 그 인자 중 하나는 디렉토리의 패스워드가 포함된 string입니다. 만약 password가 틀렸다면 password를 빠르게 알아내는 것을 방지하기 위해 3초 지연 후 호출이 실패합니다.

>
CONNECT is implemented by a loop of the form

`CONNECT`는 다음과 같은 형식의 루프로 구현됩니다.
...
>
The following trick finds a password of length n in 64n tries on the average, rather than 128<sup>n</sup>/2 (Tenex uses 7 bit characters in strings).
...
`passwordArgument`의 첫 번째 문자를 어느 페이지의 마지막 문자가 되도록 하고, 그 다음 페이지는 할당되지 않은 페이지가 되도록 배치한 후, 모든 문자를 암호의 첫 번째 문자로 시도해 봅니다.
첫 번째 문자가 페이지의 마지막 문자이고 다음 페이지가 할당되지 않도록 `passwordArgument`를 정렬합니다. 그리고 첫 번째 문자에 가능한 모든 문자를 넣어보며 시도합니다.
이 때 `CONNECT`가 `BadPassword`라고 보고하면 추측이 틀린 것이고, 시스템이 할당된 페이지에 대한 참조를 보고하면 맞은 것입니다.
이제 두 번째 문자가 페이지의 마지막 문자가 되도록 `passwordArgument`를 정렬하고, 같은 방법을 반복하면 됩니다.
...
예를 들어, 래스터 이미지를 조작하는 BitBlt 나 RasterOp 인터페이스는 Dan Ingalls가 Alto의 고해상도 대화형 디스플레이를 몇 년 동안 실험한 결과 고안해낸 것입니다.
...
One major commercial system for some time used a FindNamedField procedure that ran in time O(n<sup>2</sup>), where n is the length of the document.
...
어떤 상용 시스템에서는 길이가 n인 문서에 대해 O(n<sup>2</sup>) 시간이 걸리는 `FindNamedField` 프로시저를 사용하기도 했습니다.
...
<mark>빠르게 돌아가도록 만들 것.</mark> 범용적이거나 강력한 것보다 그게 낫습니다.
...
<mark>프로시저 인자를 사용해 인터페이스에 유연성을 제공할 것.</mark>
...
<mark>기본 인터페이스를 안정적으로 유지할 것.</mark>
...
<mark>인터페이스를 변경해야 한다면 기준점을 유지할 것.</mark>
...
<mark>어차피 버린다고 생각하고, 하나를 버릴 계획을 세울 것.</mark>
...
효율적인 프로그램을 작성하는 것은 논리적인 극한에 도전하는 연습이다. (E. Dijkstra)
...
이 프린터는 문자와 직사각형 리스트를 m × n 크기의 배열로 스캔-변환하는데,
...
이 문제를 이렇게 해결합니다.
배열 전체를 'band'라고 부르는 16 × 4200 사이즈의 비트 조각들로 쪼갭니다.
...
<mark>좋은 아이디어는 일반화하기보다 재사용할 것.</mark>
...
<mark>일반적인 케이스와 최악의 케이스는 별도 처리를 원칙으로 할 것.</mark>
왜냐하면 두 경우의 요구 사항이 상당히 다르기 때문이다.
...
고정적으로 <mark>자원을 분할해서 사용할 것</mark>.
확실하지 않은 경우라면 리소스를 공유하는 것보다 공유하지 않는 것이 낫습니다.
...
가능하다면 <mark>정적 분석을 사용할 것</mark>. 이것은 이전 슬로건을 일반화한 것입니다.
...
Updating depends on an equation of the form $$f(x + \Delta ) = g(x, \Delta , f(x))$$ in which $$g$$ is much cheaper to compute than $$f$$.
...
<mark>캐시가 응답하게 할 것.</mark>
비싼 계산을 반복하지 말고 결과를 캐시하세요.
...
캐시의 갱신은 $$f(x + \Delta ) = g(x, \Delta , f(x))$$ 형태의 식을 사용하며, 이 식에서는 $$f$$보다 $$g$$를 계산하는 비용이 훨씬 적습니다.
예를 들어, $$x$$ 는 1000개의 숫자 배열이고, $$f$$ 가 배열 요소의 합, $$\Delta$$는 그 중 하나에 대한 새로운 값, 즉 $$[i, v]$$의 쌍이라 생각해 봅시다.
...
예를 들어 Bravo 에디터는 `DisplayLine(document, firstChar)`라는 함수를 갖고 있습니다.
...
Bravo 에디터에서는 현재 화면에 표시된 각 행(화면의 위나 아래에 몇 줄이 더 있기도 합니다)에 대한 캐시 항목이 있습니다.
...
<mark>확신이 없다면, brute force를 쓸 것.</mark>
...
가능한 <mark>한 배치 처리를 사용할 것.</mark>
작업을 점진적으로 처리하는 것은 항상 더 많은 비용이 듭니다.
...
<mark>업데이트 내용을 로그로 남길 것.</mark>
객체의 상태에 대한 진실을 기록하기 위해 업데이트 로그를 사용하세요.
...
<mark>작업을 원자적으로 또는 재시작 가능하게 만들 것.</mark>
