...
updated : 2022-08-13 17:51:28 +0900
tag     : 
toc     : true
public  : true
parent  : [[/article]]
...
아마도 상당한 고맥락 대화를 나누는 느낌이었기 때문이었던 것 같다.
...
- [[/clipping/predicting-the-future/]]{앨런 케이: 미래를 예측하는 가장 좋은 방법은 미래를 만드는 것}
- [[/clipping/ewd/32]]{데이크스트라: 과학적으로 프로그래밍하자}
- [[/clipping/marvin-minsky/why-programming-is-a-good-medium]]{마빈 민스키: 프로그래밍은 확실하지 않게 이해한 대상을 표현하기 좋은 도구}
...
몇 년 전 앤디 허츠필드의 Revolution in the valley(미래를 만든 Geeks, 인사이트 출판)를 읽어보니[^insightbook] 이런 종류의 트릭을 쓴 코드를 작성했더니 [[/wiki/Bill-Atkinson-productivity]]{빌 앳킨슨}이 이건 해커들의 방법이지 책임감있는 방식이 아니라며 엄청 화를 내더라는 이야기가 나온다. 빌 앳킨슨도 이런 생각을 했던 것.
...
이건 처음에 감으로만 느끼고 있었다가 [[/wiki/REST-paper-summary]]{로이 필딩의 논문을 읽고 구체화하게 됐다}.
