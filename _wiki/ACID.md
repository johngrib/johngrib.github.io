...
updated : 2019-10-03 13:34:42 +0900
tag     : 
toc     : true
public  : true
parent  : proverb
...
* 영속성(durability): 트랜잭션의 결과는 시스템 실패로 유실되면 안된다.[^list]
...
> 트랜잭션이 완전히 실행되거나 전혀 실행되지 않음(all-or-nothing)을 의미한다.
...
* undo: 트랜잭션 프로그램이 작업 도중 실패하면, 트랜잭션이 수정한 내용을 모두 undo 한다.
    * 이를 통해 DB가 트랜잭션 일행 이전 상태로 돌아가게 보장해야 한다.

abort는 원자성의 중요한 특징이다.
...
> 어떤 트랜잭션 집합을 실행하는 시스템의 결과가
그 시스템이 이들 트랜잭션을 한 번에 하나씩 실행하는 것과 동일한 결과를 얻는 경우에
이들 트랜잭션을 고립되어 있다고 말한다.
고립성의 기술적 정의는 직렬성(serializability)이다.
...
> 영속성은 트랜잭션이 실행을 commit 할 때 정전 또는 운영체제에 장애가 발생해도 모든 업데이트가 기억장치에 저장됨을 의미한다.
<br><br>(중략)<br><br>
...
* 웹 문서
    * [Theo Harder, Andreas Reuter: "Principles of Transaction-Oriented Database Recovery", 1983][haerder-1983]: ACID가 최초로 소개된 논문.

## 주석

[^list]: 트랜잭션 처리의 원리. 1.3 원자성, 일관성, 고립성, 영속성. 26쪽.
[^tp]: Transaction Processing System
[^prin-atomic]: 트랜잭션 처리의 원리. 1.3 원자성, 일관성, 고립성, 영속성. 27쪽.
