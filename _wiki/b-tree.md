...
updated : 2019-10-16 00:00:15 +0900
tag     : db
toc     : true
public  : true
parent  : algorithm
...
다음은 위와 다른 구조와 다른 아이템을 가진 B-트리이다.
...
많은 페이지 참조를 따라가지 않아도 된다(분기 계수 500의 4KB 페이지의 4단계 트리는 256TB까지 저장할 수 있다).
[^kleppmann-insertion]

![Inserting a new key into a B-tree set]( /post-img/b-tree/insertion.png )

위의 이미지는 이 B-트리에 A를 추가하는 내용이다.
