...
updated : 2024-03-03 14:17:47 +0900
tag     : 
...
>
주의: '현대적인 대안'이라며 인기를 얻는 명령들을 조심하세요.
>
예를 들어 [exa](https://github.com/ogham/exa )는 [[/cmd/ls]]의 대안으로 한때 대단한 인기를 끌었지만,
[알 수 없는 이유로 메인테이너가 응답을 하지 않게 되면서](https://github.com/ogham/exa/issues/1243 ) [eza](https://github.com/eza-community/eza )로 fork되었습니다.
문제는 이러면서 `exa` 명령이 `eza`로 바뀌게 되었고 `.bashrc` 같은 설정 파일도 수정해야 했다는 것입니다.
이 케이스에서는 `.bashrc` 파일을 수정하는 것만으로 간단하게 해결을 할 수 있었지만 만약 `exa`를 사용하는 셸 스크립트를 많이 만들어뒀다면 스크립트를 수정하는 것 뿐 아니라, 이전과 똑같이 작동하는지 테스트하고 결과를 확인하는 데 상당한 시간이 필요했을 것입니다.
